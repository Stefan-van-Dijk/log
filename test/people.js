(function(){
  ...
  function save(id,values,self=false){
  ...
  function edit(id='',self=false){
    ...
    const d=window.LogCardsUI.sheet(self?'Mijn gegevens':id?'Persoon bewerken':'Persoon toevoegen',`<form class="people-form">${select}${field('name','Naam')}${self?'':field('relationship','Relatie (bijvoorbeeld collega, klant of familie)')}${field('organization','Organisatie')}${field('email','E-mailadres','email')}${field('phone','Telefoonnummer','tel')}<label>Kleur<input name="color" type="color" value="${esc(/^#[0-9a-f]{6}$/i.test(p.color||'')?p.color:'#4da3ff')}"></label><label>Notitie<textarea name="note" maxlength="2000">${esc(p.note||'')}</textarea></label><button class="btn primary full" type="submit">Bewaren</button></form>`);
  ...
  function show(id,self=false){
    ...
    const d=window.LogCardsUI.sheet(self?'Mijn gegevens':p.name,`<div class="people-detail"><h3>${esc(p.name)}</h3>${[['Relatie',p.relationship],['Organisatie',p.organization],['E-mailadres',p.email],['Telefoonnummer',p.phone],['Notitie',p.note]].filter(([,v])=>v).map(([label,v])=>`<div><small>${label}</small><p>${esc(v)}</p></div>`).join('')}</div><button type="button" class="btn secondary full" data-person-edit="${esc(p.id)}">Bewerken</button>`);
  ...
  function render(){
    ...
    root.innerHTML=`<section class="cards-module people-module"><div class="people-self">${row(self,true)}</div><button type="button" class="btn secondary full" data-person-add>＋ Persoon toevoegen</button><div class="people-list">${list.map(p=>row(p)).join('')||'<p class="cards-empty">Geen personen gevonden.</p>'}</div>${archives.length?`<details class="cards-content-details"><summary>Archief · ${archives.length}</summary>${archives.map(r=>`<div class="people-archive"><span>${esc(r.payload?.name||'Persoon')}</span><button type="button" class="btn secondary" data-log-time-restore="${esc(r.batchId)}">Herstel</button></div>`).join('')}</details>`:''}</section>`;
    return list.length;
  }
  ...
  function mount(target){if(root===target&&root.querySelector('.people-module'))return;root=target;signature='';root.onclick=e=>{const b=e.target.closest('button');if(!b)return;if(b.hasAttribute('data-person-open')&&Date.now()<Number(b.closest('[data-person-row]')?.dataset.suppressUntil||0))return;const self=b.hasAttribute('data-person-self');if(b.hasAttribute('data-person-add'))edit();if(b.hasAttribute('data-person-open'))show(b.dataset.personOpen,self);if(b.hasAttribute('data-person-edit'))edit(b.dataset.personEdit,self);};refresh();}
