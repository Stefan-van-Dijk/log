...
function renderSummary(t) {
  return `<section class="summary"><div class="summary-main"><div class="summary-label">Geboekte eigen tijd</div><div class="summary-value">${periodHours(t.own)}</div></div><div class="summary-parts"><div class="summary-part"><span>Personen</span><strong>${periodHours(t.colleague)}</strong></div><div class="summary-part"><span>Totale inzet</span><strong>${periodHours(t.total)}</strong></div><div class="summary-part"><span>Tussenstops</span><strong>${t.interruptions}</strong></div></div></section>`;
}
...
function openInterruptionStart() {
  ...
  const top = sortedByUsage(state.colleagues.filter(p=>String(p.id)!==String(state.settings.selfPersonId))).slice(0, 8);
  openModal(`<div class="modal-head"><h2 id="modalTitle">Tussenstop</h2><button class="close">×</button></div><p class="muted small">Kies eventueel bij wie je meekijkt. Meerdere personen zijn mogelijk.</p><div id="interruptPeople" class="check-list">${top.map((c,i)=>`<div class="check-row"><input type="checkbox" value="${c.id}" id="ic-${c.id}" ${i===0?'checked':''}><label for="ic-${c.id}">${safeText(c.name)}</label><small>${c.usageCount||0}×</small></div>`).join('') || '<div class="muted small">Nog geen personen bekend.</div>'}</div><div class="inline-form"><div class="field"><label>Andere persoon</label><input id="interruptNewColleague" placeholder="Naam"></div><button id="addInterruptColleague" class="btn small">Toevoegen</button></div><div class="field"><label>Afdeling (optioneel)</label><input id="interruptDepartment" placeholder="Bijvoorbeeld Engineering"></div><div class="field"><label>Onderwerp (optioneel)</label><select id="interruptTheme">${themeOptions('', true)}</select></div><div class="field"><label>Subthema</label><select id="interruptSub"><option value="">Geen subthema</option></select></div><div class="field"><label>Locatie (optioneel)</label><input id="interruptLocation" value="${safeText(state.timer.locationName || '')}"></div><button id="confirmInterruption" class="btn primary full">Start tussenstop</button>`);
  const refreshPeople = selected => { const set = new Set(selected); $('#interruptPeople').innerHTML = sortedByUsage(state.colleagues.filter(p=>String(p.id)!==String(state.settings.selfPersonId))).slice(0, 12).map(c=>`<div class="check-row"><input type="checkbox" value="${c.id}" id="ic-${c.id}" ${set.has(c.id)?'checked':''}><label for="ic-${c.id}">${safeText(c.name)}</label><small>${c.usageCount||0}×</small></div>`).join('') || '<div class="muted small">Nog geen personen bekend.</div>'; };
  ...
  $('#addInterruptColleague').addEventListener('click', () => { const selected = $$('#interruptPeople input:checked').map(x=>x.value); const c = addColleague($('#interruptNewColleague').value); if (!c) return; selected.push(c.id); refreshPeople(selected); $('#interruptNewColleague').value=''; toast('Persoon toegevoegd'); });
...
function colleagueSection(ownMinutes, prefix, initialAllocations = []) {
  ...
  const colleagues = sortedByUsage(state.colleagues.filter(p=>String(p.id)!==String(state.settings.selfPersonId)||selected.has(String(p.id))));
  const initialMode = selected.size ? 'individual' : 'same';
  return `<div class="settings-section"><h3>Inzet van anderen</h3><div id="${prefix}ColleagueList" class="check-list">${colleagues.map(c=>`<div class="check-row"><input type="checkbox" id="${prefix}c-${c.id}" value="${c.id}" class="${prefix}col-check" ${selected.has(String(c.id))?'checked':''}><label for="${prefix}c-${c.id}">${safeText(c.name)}</label><small>${c.usageCount||0}×</small></div>`).join('') || '<p class="muted small">Geen personen.</p>'}</div><div class="inline-form"><div class="field"><label>Nieuwe persoon</label><input id="${prefix}NewColleague"></div><button id="${prefix}AddColleague" class="btn small" type="button">Toevoegen</button></div><div id="${prefix}ColleagueTimes" style="display:none"><div class="field"><label>Verdeling</label><select id="${prefix}Mode"><option value="same" ${initialMode==='same'?'selected':''}>Zelfde als mijn geboekte tijd</option><option value="common">Eén tijd voor iedereen</option><option value="individual" ${initialMode==='individual'?'selected':''}>Per persoon</option></select></div><div id="${prefix}CommonWrap" class="field" style="display:none"><label>Minuten per persoon</label><input id="${prefix}CommonMinutes" type="number" min="0" value="${ownMinutes}"></div><div id="${prefix}IndividualWrap" style="display:none"></div></div></div>`;
}
...
function wireColleagueSection(prefix, ownMinutes, initialAllocations = []) {
  ...
  $(`#${prefix}AddColleague`).addEventListener('click',()=>{const selected=new Set(selectedIds());const c=addColleague($(`#${prefix}NewColleague`).value);if(!c)return;selected.add(c.id);list.innerHTML=sortedByUsage(state.colleagues.filter(p=>String(p.id)!==String(state.settings.selfPersonId))).map(x=>`<div class="check-row"><input type="checkbox" id="${prefix}c-${x.id}" value="${x.id}" class="${prefix}col-check" ${selected.has(x.id)?'checked':''}><label for="${prefix}c-${x.id}">${safeText(x.name)}</label></div>`).join('');$(`#${prefix}NewColleague`).value='';refresh();});
  refresh();
...
function openEntryDetail(entryId) {
  ...
  const allocationDetails = Array.isArray(e.allocations) && e.allocations.length
    ? `<div class="settings-section"><h3>Inzet van anderen</h3><div class="list">${e.allocations.map(allocation => `<div class="entry"><div class="entry-main"><strong>${safeText(allocation.colleagueName || state.colleagues.find(item => String(item.id) === String(allocation.colleagueId))?.name || 'Persoon')}</strong><small>${displayMinutes(allocation.minutes)}</small></div></div>`).join('')}</div></div>`
    : '';
  openModal(`<div class="modal-head"><h2 id="modalTitle">${e.activityType==='interruption'?'Tussenstop':'Registratie'}</h2><button class="close">×</button></div><div class="detail-hero" style="--item-accent:${themeColor(e.themeId || e.themeName)}"><div class="kicker">${safeText(e.kind||'Activiteit')}</div><h2>${safeText(e.themeName||'Activiteit')}</h2>${e.subthemeName?`<div class="muted">${safeText(e.subthemeName)}</div>`:''}</div><div class="detail-grid"><div class="detail-item"><span>Werkelijk</span><strong>${clockMinutes(e.actualMinutes)}</strong></div><div class="detail-item"><span>Geboekt</span><strong>${displayMinutes(e.ownMinutes)}</strong></div>${e.deductedInterruptionMinutes?`<div class="detail-item"><span>Aftrek tussenstops</span><strong>${clockMinutes(e.deductedInterruptionMinutes)}</strong></div>`:''}${e.activityType==='interruption'?`<div class="detail-item"><span>Aftrek hoofdactiviteit</span><strong>${clockMinutes(e.deductMinutes)}</strong></div>`:''}<div class="detail-item"><span>Inzet van anderen</span><strong>${displayMinutes(e.colleagueMinutes)}</strong></div><div class="detail-item"><span>Totale inzet</span><strong>${displayMinutes(e.totalMinutes)}</strong></div></div><div class="settings-section"><h3>Informatie</h3><div class="list"><div class="entry"><div class="entry-main"><strong>Datum</strong><small>${dateText(e.dateISO)}</small></div></div>${e.startISO?`<div class="entry"><div class="entry-main"><strong>Tijd</strong><small>${timeText(e.startISO)}–${timeText(e.endISO)}</small></div></div>`:''}${e.locationName?`<div class="entry"><div class="entry-main"><strong>Locatie</strong><small>${safeText(e.locationName)}</small></div></div>`:''}${e.departmentName?`<div class="entry"><div class="entry-main"><strong>Afdeling</strong><small>${safeText(e.departmentName)}</small></div></div>`:''}${e.people?.length?`<div class="entry"><div class="entry-main"><strong>Bij / met</strong><small>${safeText(e.people.map(p=>p.name).join(', '))}</small></div></div>`:''}${e.note?`<div class="entry"><div class="entry-main"><strong>Notitie</strong><small>${safeText(e.note)}</small></div></div>`:''}</div></div>${allocationDetails}${children.length?`<div class="settings-section"><h3>Tussenstops</h3><div class="list">${children.map(c=>entryRow(c,true)).join('')}</div></div>`:''}<div class="row"><button id="editEntry" class="btn primary">Wijzigen</button><button id="deleteEntry" class="btn danger">Verwijderen</button></div>`);
...
function renderSettingsPage() {
  ...
  $('#main').innerHTML = `<section class="settings-page">${settingsAccordion('Tijd en afronding', `${displayMinutes(state.settings.roundingUnitMinutes)} · ${state.settings.roundingMode==='none'?'geen afronding':'afronding actief'}`, timeBody)}${settingsAccordion('Bediening', 'Swipe en verwijderen', controlBody)}${settingsAccordion('Tussenstops', `${state.settings.interruptionUnitMinutes} minuten`, interruptionBody)}${settingsAccordion('Gegevensbeheer', 'Tijdregistratie wissen', dataBody)}</section>`;
  ...
  ['timeDisplay','roundMode','roundUnit','roundThreshold','interruptUnit','interruptThreshold'].forEach(id=>$('#'+id).addEventListener('change',()=>{saveSettings();toast('Instelling bewaard');})); $('#resetData').addEventListener('click',resetAll);
}
...
async function resetAll() {
  ...
  const plan = policy?.createPlan({
    ...
    owned: [
      ...
      state.colleagues.length ? { key: 'colleagues', label: state.colleagues.length === 1 ? 'collega' : "personen", count: state.colleagues.length } : null
    ],
    incoming: []
  });
